...
module.exports = function Updater() {
    ...
    async function getReverseRegistrar(name) {
        ...
        const reverseRegistrarAddress = await registry.owner(reverseNode)
    ...
    async function getRegistrant() {
        ...
        if (labels.length === 2) {
            ...
            try {
                ...
                // ERC721 "ownerOf" throws when owner is not set. Catch this error and return 'undefined'
                // to indicate that the name has no owner
                return undefined
            }
        } else {
            return undefined
    ...
    async function setup(options) {
        ...
        if (ensName) {
            ...
            resolver = await getResolver(node)
        }
    }
    ...
    async function getReverseName(address) {
        ...
        if (reverseResolverAddress === zeroAddress) {
            verbose && console.log(`No reverse resolver set for ${address}`)
            return ''
        }
    ...
    async function setReverseName(reverseName) {
        ...
        try {
            ...
            throw Error(`Error performing setName(): ${error}`)
        }
    }
